...
class Queue:
														# Created a new constructor class
	def __init__(self,maxsize):
	...
	def Enqueue(self,key):
		if self.stack1.isFull() is False:			#check if Full
			self.stack1.push(key)					#push to stack

	def Dequeue(self):
		if self.stack1.isEmpty() is False:			#check if Empty
			for i in range(self.maxsize):			
				self.stack2.push(self.stack1.pop())	#copy stack 1 into stack 2
			temp = self.stack2.pop()				#save the top of stack 2 into temp
			for i in range(self.maxsize):
				self.stack1.push(self.stack2.pop())	#copy the remaining stack 2 into stack 1
			return temp								#return the top of stack 2 
